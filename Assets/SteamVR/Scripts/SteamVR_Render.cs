...
namespace SteamVR.Scripts {
	public class SteamVR_Render : MonoBehaviour {
		...
		static public void Add(SteamVR_Camera vrcam) { if (!isQuitting) { instance.AddInternal(vrcam); } }

		static public void Remove(SteamVR_Camera vrcam) { if (!isQuitting && _instance != null) { instance.RemoveInternal(vrcam); } }
		...
		private IEnumerator RenderLoop() {
			while (true) {
				...
				if (cameraMask != null) { cameraMask.Clear(); }
#endif
			}
		}
		...
		void RenderEye(SteamVR vr, EVREye eye) {
			...
			if (cameraMask != null) { cameraMask.Set(vr, eye); }
		...
		private void OnInputFocus(params object[] args) {
			...
			if (hasFocus) {
				if (pauseGameWhenDashboardIsVisible) { Time.timeScale = timeScale; }
				...
				for (int i = 0; i < 64; i++) {
					...
					switch ((EVREventType) vrEvent.eventType) {
						case EVREventType.VREvent_InputFocusCaptured: // another app has taken focus (likely dashboard)
							if (vrEvent.data.process.oldPid == 0) { SteamVR_Utils.Event.Send("input_focus", false); }
							break;
						case EVREventType.VREvent_InputFocusReleased: // that app has released input focus
							if (vrEvent.data.process.pid == 0) { SteamVR_Utils.Event.Send("input_focus", true); }
							break;
						...
						default:
							...
							if (name != null) { SteamVR_Utils.Event.Send(name.Substring(8) /*strip VREvent_*/, vrEvent); }
							break;
					}
				}
			}

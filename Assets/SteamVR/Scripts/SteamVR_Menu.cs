...
namespace SteamVR.Scripts {
	public class SteamVR_Menu : MonoBehaviour {
		...
		void OnGUI() {
			...
			if (background != null) { GUI.DrawTexture(new Rect((area.width - background.width)/2, (area.height - background.height)/2, background.width, background.height), background); }
			...
			GUILayout.Label("Scale limits:");
			{
				...
				if (result != scaleLimitX) { if (float.TryParse(result, out scaleLimits.x)) { scaleLimitX = result; } }
			}
			{
				var result = GUILayout.TextField(scaleLimitY);
				if (result != scaleLimitY) { if (float.TryParse(result, out scaleLimits.y)) { scaleLimitY = result; } }
			}
			GUILayout.EndHorizontal();
			...
			GUILayout.Label("Scale rate:");
			{
				...
				if (result != scaleRateText) { if (float.TryParse(result, out scaleRate)) { scaleRateText = result; } }
			}
			GUILayout.EndHorizontal();
			...
			if (tracker != null) {
				...
				if (render.trackingSpace == ETrackingUniverseOrigin.TrackingUniverseSeated) {
					if (GUILayout.Button("Switch to Standing")) { render.trackingSpace = ETrackingUniverseOrigin.TrackingUniverseStanding; }
					if (GUILayout.Button("Center View")) {
						var system = OpenVR.System;
						if (system != null) { system.ResetSeatedZeroPose(); }
					}
				} else { if (GUILayout.Button("Switch to Seated")) { render.trackingSpace = ETrackingUniverseOrigin.TrackingUniverseSeated; } }
			}
		...
		public void ShowMenu() {
			...
			var tracker = SteamVR_Render.Top();
			if (tracker != null) { scale = tracker.origin.localScale.x; }
		}
		...
		void Update() { if (Input.GetKeyDown(KeyCode.Escape) || Input.GetKeyDown(KeyCode.Joystick1Button7)) { if (overlay == null) { ShowMenu(); } else { HideMenu(); } } else if (Input.GetKeyDown(KeyCode.Home)) { SetScale(1.0f); } else if (Input.GetKey(KeyCode.PageUp)) { SetScale(Mathf.Clamp(scale + scaleRate*Time.deltaTime, scaleLimits.x, scaleLimits.y)); } else if (Input.GetKey(KeyCode.PageDown)) { SetScale(Mathf.Clamp(scale - scaleRate*Time.deltaTime, scaleLimits.x, scaleLimits.y)); } }

		void SetScale(float scale) {
			this.scale = scale;

			var tracker = SteamVR_Render.Top();
			if (tracker != null) { tracker.origin.localScale = new Vector3(scale, scale, scale); }
		}

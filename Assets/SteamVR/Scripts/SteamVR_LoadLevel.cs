...
namespace SteamVR.Scripts {
	public class SteamVR_LoadLevel : MonoBehaviour {
		...
		public static void Begin(string levelName, bool showGrid = false, float fadeOutTime = 0.5f, float r = 0.0f, float g = 0.0f, float b = 0.0f, float a = 1.0f) {
		...
		void OnGUI() {
			...
			if (progressBarEmpty != null && progressBarFull != null) {
				if (progressBarOverlayHandle == OpenVR.k_ulOverlayHandleInvalid) { progressBarOverlayHandle = GetOverlayHandle("progressBar", progressBarTransform != null ? progressBarTransform : transform, progressBarWidthInMeters); }
	...
	// Draw loading screen and progress bar to 2d companion window as well.
		...
		IEnumerator LoadLevel() {
			...
			if (loadExternalApp) {
				...
				if (applications == null) { Debug.Log("Failed to get OpenVR.Applications interface!"); } else {
				...
				} else { SceneManager.LoadScene(levelName, mode); }
#else
			if (loadAsync)
			{

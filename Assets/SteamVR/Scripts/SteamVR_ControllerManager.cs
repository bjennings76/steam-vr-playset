...
namespace SteamVR.Scripts {
	public class SteamVR_ControllerManager : MonoBehaviour {
		...
		readonly bool[] connected = new bool[OpenVR.k_unMaxTrackedDeviceCount]; // controllers only
		...
		void OnEnable() {
			...
			for (int i = 0; i < SteamVR.connected.Length; i++) { if (SteamVR.connected[i]) { OnDeviceConnected(i, true); } }
		...
		static readonly string[] labels = {"left", "right"};
		...
		private void ShowObject(Transform t, string name) {
			var hidden = t.parent;
			if (hidden.gameObject.name != name) { return; }
		...
		private void SetTrackedDeviceIndex(int objectIndex, uint trackedDeviceIndex) {
			...
			if (trackedDeviceIndex != indices[objectIndex]) {
				...
				if (obj != null) {
					if (trackedDeviceIndex == OpenVR.k_unTrackedDeviceIndexInvalid) { obj.SetActive(false); } else {
						obj.SetActive(true);
		...
		private void OnTrackedDeviceRoleChanged(params object[] args) { Refresh(); }
			...
			if (leftIndex == OpenVR.k_unTrackedDeviceIndexInvalid && rightIndex == OpenVR.k_unTrackedDeviceIndexInvalid) {
				...
				if (leftIndex != OpenVR.k_unTrackedDeviceIndexInvalid && rightIndex != OpenVR.k_unTrackedDeviceIndexInvalid) {
					for (uint deviceIndex = 0; deviceIndex < connected.Length; deviceIndex++) {
						...
						if (!connected[deviceIndex]) { continue; }

						if (deviceIndex != leftIndex && deviceIndex != rightIndex) { SetTrackedDeviceIndex(objectIndex++, deviceIndex); }
					}
				}
			}

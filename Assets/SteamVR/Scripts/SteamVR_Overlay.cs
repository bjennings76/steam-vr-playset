	...
	public class SteamVR_Overlay : MonoBehaviour {
		...
		public void UpdateOverlay() {
			...
			if (texture != null) {
				...
				if (error == EVROverlayError.InvalidHandle || error == EVROverlayError.UnknownOverlay) { if (overlay.FindOverlay(key, ref handle) != EVROverlayError.None) { return; } }
				...
				if (curved || antialias) { highquality = true; }
				...
				} else if (overlay.GetHighQualityOverlay() == handle) { overlay.SetHighQualityOverlay(OpenVR.k_ulOverlayHandleInvalid); }

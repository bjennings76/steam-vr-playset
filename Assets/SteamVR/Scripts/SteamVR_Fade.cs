...
namespace SteamVR.Scripts {
	public class SteamVR_Fade : MonoBehaviour {
		...
		static public void Start(Color newColor, float duration, bool fadeOverlay = false) { SteamVR_Utils.Event.Send("fade", newColor, duration, fadeOverlay); }

		static public void View(Color newColor, float duration) {
			var compositor = OpenVR.Compositor;
			if (compositor != null) { compositor.FadeToColor(duration, newColor.r, newColor.g, newColor.b, newColor.a, false); }
		}
	...
	void Update()
	{
		...
		void OnEnable() {
			if (fadeMaterial == null) { fadeMaterial = new Material(Shader.Find("Custom/SteamVR_Fade")); }

			SteamVR_Utils.Event.Listen("fade", OnStartFade);
			SteamVR_Utils.Event.Send("fade_ready");
		}

		void OnDisable() { SteamVR_Utils.Event.Remove("fade", OnStartFade); }
		...
		void OnPostRender() {
			if (currentColor != targetColor) {
				...
				} else { currentColor += deltaColor*Time.deltaTime; }

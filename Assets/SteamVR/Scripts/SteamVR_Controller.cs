	...
	public class SteamVR_Controller {
		...
		public class Device {
			...
			void UpdateHairTrigger() {
				...
				if (hairTriggerState) { if (value < hairTriggerLimit - hairTriggerDelta || value <= 0.0f) { hairTriggerState = false; } } else { if (value > hairTriggerLimit + hairTriggerDelta || value >= 1.0f) { hairTriggerState = true; } }
		...
		public static void Update() { for (int i = 0; i < OpenVR.k_unMaxTrackedDeviceCount; i++) { Input(i).Update(); } }
		...
		public static int GetDeviceIndex(DeviceRelation relation, ETrackedDeviceClass deviceClass = ETrackedDeviceClass.Controller, int relativeTo = (int) OpenVR.k_unTrackedDeviceIndex_Hmd) // use -1 for absolute tracking space
		{
			var result = -1;

			var invXform = ((uint) relativeTo < OpenVR.k_unMaxTrackedDeviceCount) ? Input(relativeTo).transform.GetInverse() : SteamVR_Utils.RigidTransform.identity;
			...
			for (int i = 0; i < OpenVR.k_unMaxTrackedDeviceCount; i++) {
				...
				if (relation == DeviceRelation.FarthestRight) { score = pos.x; } else if (relation == DeviceRelation.FarthestLeft) { score = -pos.x; } else {
					...
					if (relation == DeviceRelation.Leftmost) { score = (cross.y > 0.0f) ? 2.0f - dot : dot; } else { score = (cross.y < 0.0f) ? 2.0f - dot : dot; }
				}

	...
	public class SteamVR_SkyboxEditor : UnityEditor.Editor {
		...
		public override void OnInspectorGUI() {
			...
			if (!AssetDatabase.IsValidFolder(assetPath))
			{
...
#endif
				var directions = new[] {Quaternion.LookRotation(Vector3.forward), Quaternion.LookRotation(Vector3.back), Quaternion.LookRotation(Vector3.left), Quaternion.LookRotation(Vector3.right), Quaternion.LookRotation(Vector3.up, Vector3.back), Quaternion.LookRotation(Vector3.down, Vector3.forward)};
				...
				var textures = new[] {new Texture2D(width, height, TextureFormat.ARGB32, false), new Texture2D(width, height, TextureFormat.ARGB32, false)};
				...
				foreach (SteamVR_Skybox target in targets) {
					...
					if (camera != tempCamera) {
						...
						target.transform.localPosition = oldPosition;
						target.transform.localRotation = oldRotation;
					} else { tempCamera.targetTexture = null; }

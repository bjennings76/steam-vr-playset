...
namespace SteamVR.Editor {
	...
	public class SteamVR_Update : EditorWindow {
		...
		static void Update() {
			if (wwwVersion != null) {
				if (!wwwVersion.isDone) { return; }

				if (UrlSuccess(wwwVersion)) { version = wwwVersion.text; }
			...
			if (wwwNotes != null) {
				if (!wwwNotes.isDone) { return; }

				if (UrlSuccess(wwwNotes)) { notes = wwwNotes.text; }

				wwwNotes = null;

				if (notes != "") { window.Repaint(); }
			}
		...
		static bool UrlSuccess(WWW www) {
			if (!string.IsNullOrEmpty(www.error)) { return false; }
			if (Regex.IsMatch(www.text, "404 not found", RegexOptions.IgnoreCase)) { return false; }
			return true;
		}
		...
		static bool ShouldDisplay() {
			...
			if (EditorPrefs.HasKey(string.Format(doNotShowKey, version))) { return false; }
			...
			for (int i = 0; i < versionSplit.Length && i < currentVersionSplit.Length; i++) {
				...
				if (int.TryParse(versionSplit[i], out versionValue) && int.TryParse(currentVersionSplit[i], out currentVersionValue)) {
					if (versionValue > currentVersionValue) { return true; }
					if (versionValue < currentVersionValue) { return false; }
				}
			}

			// same up to this point, now differentiate based on number of sub values (e.g. 1.0.4.1 vs 1.0.4)
			if (versionSplit.Length <= currentVersionSplit.Length) { return false; }
		...
		var logo = Resources.LoadAssetAtPath<Texture2D>(resourcePath + "logo.png");
			...
			if (EditorGUI.EndChangeCheck()) {
				...
				if (doNotShow) { EditorPrefs.SetBool(key, true); } else { EditorPrefs.DeleteKey(key); }
			}
		}
	}
}
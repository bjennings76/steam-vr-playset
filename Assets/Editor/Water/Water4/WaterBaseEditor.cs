using Standard_Assets.Environment.Water.Water.Scripts;
using UnityEditor;
using UnityEngine;

namespace Editor.Water.Water4 {
	...
	public class WaterBaseEditor : UnityEditor.Editor {
		...
		public override void OnInspectorGUI() {
			...
			if (!oceanMaterial) {
				sharedMaterial.objectReferenceValue = WaterEditorUtility.LocateValidWaterMaterial(oceanBase.transform);
			...
			if (waterQuality.intValue > (int) WaterQuality.Low && !SystemInfo.supportsRenderTextures) { EditorGUILayout.HelpBox("Water features not supported", MessageType.Warning); }
			if (edgeBlend.boolValue && !SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.Depth)) { EditorGUILayout.HelpBox("Edge blend not supported", MessageType.Warning); }
			...
			if (hasShore) { WaterEditorUtility.SetMaterialTexture("_ShoreTex", (Texture) EditorGUILayout.ObjectField("Shore & Foam", WaterEditorUtility.GetMaterialTexture("_ShoreTex", waterBase.sharedMaterial), typeof (Texture), false), waterBase.sharedMaterial); }
			...
			if (!oceanMaterial.HasProperty("_Fresnel")) {
				...
				Texture fresnelTex = (Texture) EditorGUILayout.ObjectField("Ramp", WaterEditorUtility.GetMaterialTexture("_Fresnel", oceanMaterial), typeof (Texture), false);
			...
			if (edgeBlend.boolValue) {
				...
				if (hasShore) { fade.y = EditorGUILayout.Slider("Shore fade", fade.y, 0.001f, 3.0f); }

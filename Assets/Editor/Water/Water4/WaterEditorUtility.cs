...
namespace Editor.Water.Water4 {
	class WaterEditorUtility {
		...
		public static Material LocateValidWaterMaterial(Transform parent) {
			if (parent.GetComponent<Renderer>() && parent.GetComponent<Renderer>().sharedMaterial) { return parent.GetComponent<Renderer>().sharedMaterial; }
			foreach (Transform t in parent) { if (t.GetComponent<Renderer>() && t.GetComponent<Renderer>().sharedMaterial) { return t.GetComponent<Renderer>().sharedMaterial; } }
			return null;
		}

...
namespace Editor.ImageEffects {
	...
	class CameraMotionBlurEditor : UnityEditor.Editor {
		...
		public override void OnInspectorGUI() {
			...
			if (filterType.enumValueIndex == 3 && !(target as CameraMotionBlur).Dx11Support()) { EditorGUILayout.HelpBox("DX11 mode not supported (need shader model 5)", MessageType.Info); }
			...
			} else { EditorGUILayout.Slider(maxVelocity, 2.0f, 10.0f, new GUIContent(" Velocity Max")); }

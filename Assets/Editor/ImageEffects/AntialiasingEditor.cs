...
namespace Editor.ImageEffects {
	...
	public class AntialiasingEditor : UnityEditor.Editor {
		...
		public override void OnInspectorGUI() {
			...
			if (null == mat && (target as Antialiasing).enabled) { EditorGUILayout.HelpBox("This AA technique is currently not supported. Choose a different technique or disable the effect and use MSAA instead.", MessageType.Warning); }
			...
			} else if (mode.enumValueIndex == (int) AAMode.DLAA) { EditorGUILayout.PropertyField(dlaaSharp, new GUIContent("Sharp")); } else if (mode.enumValueIndex == (int) AAMode.FXAA3Console) {

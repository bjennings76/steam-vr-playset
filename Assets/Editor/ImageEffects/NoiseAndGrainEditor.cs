...
namespace Editor.ImageEffects {
	...
	class NoiseAndGrainEditor : UnityEditor.Editor {
		...
		public override void OnInspectorGUI() {
			...
			if (dx11Grain.boolValue && !(target as NoiseAndGrain).Dx11Support()) { EditorGUILayout.HelpBox("DX11 mode not supported (need DX11 GPU and enable DX11 in PlayerSettings)", MessageType.Info); }
			...
			if (!dx11Grain.boolValue) {
				...
				} else { EditorGUILayout.PropertyField(monochromeTiling, new GUIContent(" Tiling")); }
			}

...
namespace Editor.ImageEffects {
	...
	class ColorCorrectionCurvesEditor : UnityEditor.Editor {
		...
		void CurveGui(string name, SerializedProperty animationCurve, Color color) {
			...
			if (GUI.changed) { applyCurveChanges = true; }
		}

		void BeginCurves() { applyCurveChanges = false; }
		...
		public override void OnInspectorGUI() {
			...
			if (mode.intValue > 0) { useDepthCorrection.boolValue = true; } else { useDepthCorrection.boolValue = false; }
			...
			if (!applyCurveChanges) { serObj.ApplyModifiedProperties(); }
		}
	}
}
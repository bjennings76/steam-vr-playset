...
namespace Editor.ImageEffects {
	...
	class ColorCorrectionLookupEditor : UnityEditor.Editor {
		...
		void OnEnable() { serObj = new SerializedObject(target); }
		...
		public override void OnInspectorGUI() {
			...
			if (tex && (target as ColorCorrectionLookup).basedOnTempTex != AssetDatabase.GetAssetPath(tex)) {
				...
				if (!(target as ColorCorrectionLookup).ValidDimensions(tex)) { EditorGUILayout.HelpBox("Invalid texture dimensions!\nPick another texture or adjust dimension to e.g. 256x16.", MessageType.Warning); } else if (GUILayout.Button("Convert and Apply")) {

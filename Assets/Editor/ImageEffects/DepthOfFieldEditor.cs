...
namespace Editor.ImageEffects {
	...
	class DepthOfFieldEditor : UnityEditor.Editor {
		...
		readonly AnimBool showFocalDistance = new AnimBool();
		readonly AnimBool showDiscBlurSettings = new AnimBool();
		readonly AnimBool showDX11BlurSettings = new AnimBool();
		readonly AnimBool showNearBlurOverlapSize = new AnimBool();

		bool useFocalDistance {
			get {
		...
		public override void OnInspectorGUI() {
			...
			if (EditorGUILayout.BeginFadeGroup(showFocalDistance.faded)) { EditorGUILayout.PropertyField(focalLength, new GUIContent(" Focal Distance")); }
			...
			if (!(target as DepthOfField).Dx11Support() && blurType.enumValueIndex > 0) { EditorGUILayout.HelpBox("DX11 mode not supported (need shader model 5)", MessageType.Info); }

			if (EditorGUILayout.BeginFadeGroup(showDiscBlurSettings.faded)) { EditorGUILayout.PropertyField(blurSampleCount, new GUIContent(" Sample Count")); }
			EditorGUILayout.EndFadeGroup();
			...
			if (EditorGUILayout.BeginFadeGroup(showNearBlurOverlapSize.faded)) { EditorGUILayout.Slider(foregroundOverlap, 0.1f, 2.0f, new GUIContent("  Overlap Size")); }
			EditorGUILayout.EndFadeGroup();

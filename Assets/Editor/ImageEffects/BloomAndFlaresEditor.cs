...
namespace Editor.ImageEffects {
	...
	class BloomAndFlaresEditor : UnityEditor.Editor {
		...
		public override void OnInspectorGUI() {
			...
			if (cam != null) { if (screenBlendMode.enumValueIndex == 0 && ((cam.hdr && hdr.enumValueIndex == 0) || (hdr.enumValueIndex == 1))) { EditorGUILayout.HelpBox("Screen blend is not supported in HDR. Using 'Add' instead.", MessageType.Info); } }

			if (1 == tweakMode.intValue) { EditorGUILayout.PropertyField(lensflares, new GUIContent("Cast lens flares")); }
			...
			if (1 == tweakMode.intValue) { useSrcAlphaAsMask.floatValue = EditorGUILayout.Slider(new GUIContent("Use alpha mask", "Make alpha channel define glowiness"), useSrcAlphaAsMask.floatValue, 0.0f, 1.0f); } else { useSrcAlphaAsMask.floatValue = 0.0f; }
			...
			if (1 == tweakMode.intValue) {
				...
				if (lensflares.boolValue) {
					...
					if (0 != tweakMode.intValue) { EditorGUILayout.PropertyField(lensflareMode, new GUIContent("Lens flare mode")); } else { lensflareMode.enumValueIndex = 0; }
			...
			} else { lensflares.boolValue = false; // disable lens flares in simple tweak mode
			}

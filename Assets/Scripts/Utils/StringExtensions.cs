...
public static class StringExtensions {
	...
	public static string EmptyAsNull(this string source) { return source == null || source.Equals("") ? null : source; }

	public static string[] Split(this string str, string sep) {
		return str.Split(new[] {sep}, StringSplitOptions.RemoveEmptyEntries);
	}

	public static string AggregateString<T>(this IEnumerable<T> ie,
																					string sep = ",",
																					Func<T, string> xfrm = null,
																					string prepend = "",
																					string append = "") {
		if (xfrm == null) { xfrm = v => v.ToString(); }
		return
			ie.Aggregate(new StringBuilder(prepend),
									(c, n) => c.Length > prepend.Length ? c.Append(sep).Append(xfrm(n)) : c.Append(xfrm(n)))
				.Append(append)
				.ToString();
	}
	...
	public static char Head(this string str) { return !string.IsNullOrEmpty(str) ? str[0] : (char) 0; }
	public static string Tail(this string str) { return !string.IsNullOrEmpty(str) ? str.Substring(1) : null; }
	public static char Last(this string str) { return !string.IsNullOrEmpty(str) ? str[str.Length - 1] : (char) 0; }
	...
	public static string Sample(this string str, int chars, string ellipsis = "...", char pad = (char) 0) {
		...
		if (pad > (char) 0) { l = l.PadRight(ml, pad); }
	...
	public static string ToTitleCase(this string text) {
		...
		foreach (Match match in Regex.Matches(text, @"\w+", RegexOptions.IgnoreCase)) {
			...
			string change = Regex.IsMatch(match.Value, @"\b(a|an|the|at|by|for|in|of|on|to|up|and|as|but|it|or|nor|with)\b",
																		RegexOptions.IgnoreCase)
												? match.Value.ToLower()
												: match.Value.ToCapitalized();
	...
	public static string ToUnderscored(this string text) {
		return
			Regex.Replace(text, @"(?:(?<!\b)([A-Z])|\s+)",
										m => "_" + (m.Captures.Count > 0 ? m.Captures[1].ToString().ToLower() : ""), RegexOptions.Singleline)
					.ToLower();
	}
	...
	public static string ToStringSafe(this object src) { return src == null ? "" : src.ToString(); }
	...
	public static bool MatchMultipartString(string input, string match) {
		...
		for (int i = 0; i < pieces.Length; i++) {
			if (input.IndexOf(pieces[i], StringComparison.OrdinalIgnoreCase) == -1) { return false; }
		}
		return true;
	}
}
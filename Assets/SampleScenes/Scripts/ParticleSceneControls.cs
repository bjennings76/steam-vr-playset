...
namespace SampleScenes.Scripts {
	public class ParticleSceneControls : MonoBehaviour {
		...
		private void Update() {
			...
			sceneCamera.localPosition = Vector3.SmoothDamp(sceneCamera.localPosition, Vector3.forward*-s_Selected.camOffset, ref m_CamOffsetVelocity, 1);
				...
				if (Physics.Raycast(ray, out hit)) {
					...
					if ((pos - m_LastPos).magnitude > s_Selected.minDist) {
						if (s_Selected.mode != Mode.Trail || m_Instance == null) {
							...
							if (m_ParticleMultiplier != null) { m_Instance.GetComponent<ParticleSystemMultiplier>().multiplier = multiply; }
		...
		private void Select(int i) {
			...
			m_Instance = null;
			foreach (var otherEffect in demoParticles.items) { if ((otherEffect != s_Selected) && (otherEffect.mode == Mode.Activate)) { otherEffect.transform.gameObject.SetActive(false); } }

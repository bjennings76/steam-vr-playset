...
namespace Elf.Script {
	public class Elf_Manager : MonoBehaviour {
		...
		public void Reset() {
			...
			for (int i = 0; i < ElfChar.Length; i++) { ElfChar[i].SetActive(false); }
		...
		void OnGUI() {
			...
			if (GUI.Button(new Rect(80, 30, 25, 25), "<")) { prevChar(); }
			GUI.Label(new Rect(110, 30, 100, 20), ElfChar[iElf].name);
			if (GUI.Button(new Rect(160, 30, 25, 25), ">")) { nextChar(); }
			// Ani
			GUI.Label(new Rect(10, 60, 100, 20), "Animation");
			if (GUI.Button(new Rect(80, 60, 25, 25), "<")) { prevAni(); }
			GUI.Label(new Rect(110, 60, 100, 20), aniname[iani]);
			if (GUI.Button(new Rect(160, 60, 25, 25), ">")) { nextAni(); }
			//Wand
			GUI.Label(new Rect(10, 90, 100, 20), "Wand");
			if (GUI.Button(new Rect(80, 90, 25, 25), "<")) { prevWand(); }
			GUI.Label(new Rect(110, 90, 100, 20), Wand[iWand].name);
			if (GUI.Button(new Rect(160, 90, 25, 25), ">")) { nextWand(); }

			//Pet
			GUI.Label(new Rect(10, 120, 100, 20), "Pet");
			if (GUI.Button(new Rect(80, 120, 25, 25), "<")) { prevPet(); }
			GUI.Label(new Rect(110, 120, 100, 20), Pet[iPet].name);
			if (GUI.Button(new Rect(160, 120, 25, 25), ">")) { nextPet(); }

			if (GUI.Button(new Rect(10, 150, 60, 25), "Reset")) { Reset(); }
		}
		...
		private void prevChar() {
			...
			for (int i = 0; i < ElfChar.Length; i++) { if (iElf != i) { ElfChar[i].SetActive(false); } }
		...
		private void prevAni() {
			...
			if (iani < 1) {
				for (int i = 0; i < ElfWand.Length; i++) { ElfWand[i].SetActive(false); }
				for (int i = 0; i < ElfPet.Length; i++) { ElfPet[i].SetActive(false); }
			} else if (iani > 0) {
				for (int i = 0; i < ElfWand.Length; i++) { if (iWand != i) { ElfWand[i].SetActive(false); } }
				for (int i = 0; i < ElfPet.Length; i++) { if (iPet != i) { ElfPet[i].SetActive(false); } }
				for (int i = 0; i < ElfChar.Length; i++) { if (iElf != i) { ElfChar[i].SetActive(false); } }
		...
		private void prevWand() {
			...
			for (int i = 0; i < Wand.Length; i++) { if (iWand != i) { Wand[i].SetActive(false); } }
			if (iani > 0) {
				for (int i = 0; i < ElfWand.Length; i++) { if (iWand != i) { ElfWand[i].SetActive(false); } }
				for (int i = 0; i < ElfPet.Length; i++) { if (iPet != i) { ElfPet[i].SetActive(false); } }
				for (int i = 0; i < ElfChar.Length; i++) { if (iElf != i) { ElfChar[i].SetActive(false); } }
		...
		private void prevPet() {
			...
			for (int i = 0; i < Pet.Length; i++) { if (iPet != i) { Pet[i].SetActive(false); } }
			if (iani > 0) {
				for (int i = 0; i < ElfWand.Length; i++) { if (iWand != i) { ElfWand[i].SetActive(false); } }
				for (int i = 0; i < ElfPet.Length; i++) { if (iPet != i) { ElfPet[i].SetActive(false); } }
				for (int i = 0; i < ElfChar.Length; i++) { if (iElf != i) { ElfChar[i].SetActive(false); } }
